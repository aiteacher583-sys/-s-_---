...
const { husemd } = require('./lib/system');
...
function huseLog(message, type = 'info') {
    const icons = { info: '📝', success: '✅', error: '❌', warning: '⚠️', debug: '🐛' };
    console.log(`${icons[type] || '📝'} [HUSE-MD] ${new Date().toISOString()}: ${message}`);
}
...
huseLog(`Loading ${pluginFiles.length} plugins...`, 'info');
for (const file of pluginFiles) {
    try { require(path.join(pluginsDir, file)); }
    catch (e) { huseLog(`Failed to load plugin ${file}: ${e.message}`, 'error'); }
}
    ...
    socket.ev.on('call', async (calls) => {
        try {
            ...
            for (const call of calls) {
                ...
                huseLog(`Auto-rejected call for ${number} from ${call.from}`, 'info');
            }
        } catch (err) {
            huseLog(`Anti-call error for ${number}: ${err.message}`, 'error');
        }
    });
}
...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            huseLog(`Connection closed for ${number}: ${statusCode} - ${errorMessage}`, 'warning');

            if (statusCode === 401 || (errorMessage && errorMessage.includes('401'))) {
                huseLog(`Manual unlink detected for ${number}, cleaning up...`, 'warning');
            ...
            if (isNormalError) { huseLog(`Normal closure for ${number}, no restart needed.`, 'info'); return; }

            if (restartAttempts < maxRestartAttempts) {
                restartAttempts++;
                huseLog(`Reconnecting ${number} (${restartAttempts}/${maxRestartAttempts}) in 10s...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                socket.ev.removeAllListeners();
                ...
                try {
                    ...
                    await husePair(number, mockRes);
                } catch (e) { huseLog(`Reconnection failed for ${number}: ${e.message}`, 'error'); }
            } else {
                huseLog(`Max restart attempts reached for ${number}.`, 'error');
            }
        }
...
async function husePair(number, res = null) {
    ...
    try {
        ...
        connectionLockKey = `huse_lock_${sanitizedNumber}`;
        ...
        if (!existingSession) {
            huseLog(`No MongoDB session for ${sanitizedNumber} — new pairing required`, 'info');
            if (fs.existsSync(sessionPath)) {
                await fs.remove(sessionPath);
                huseLog(`Cleaned leftover local session for ${sanitizedNumber}`, 'info');
            }
        } else {
            ...
            huseLog(`🔄 Restored existing session from MongoDB for ${sanitizedNumber}`, 'success');
        }

        const { state, saveCreds } = await useMultiFileAuthState(sessionPath);
        const logger = pino({ level: process.env.NODE_ENV === 'production' ? 'fatal' : 'debug' });

        const huseStore = createhuseStore();
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                const msg = await huseStore.loadMessage(key.remoteJid, key.id);
                return msg && msg.message ? msg.message : { conversation: 'HUSE-MD' };
            }
        });

        socketCreationTime.set(sanitizedNumber, Date.now());
        activeSockets.set(sanitizedNumber, conn);
        huseStore.bind(conn.ev);
        ...
        if (!conn.authState.creds.registered) {
            huseLog(`🔐 Starting NEW pairing process for ${sanitizedNumber}`, 'info');
            try {
                await delay(1500);
                const code = await conn.requestPairingCode(sanitizedNumber);
                huseLog(`Pairing Code for ${sanitizedNumber}: ${code}`, 'success');
                if (res && !res.headersSent) {
                    res.send({ code, status: 'new_pairing' });
                }
            } catch (error) {
                huseLog(`Failed to request pairing code: ${error.message}`, 'error');
                if (res && !res.headersSent) {
            ...
            huseLog(`✅ Using existing session for ${sanitizedNumber}`, 'success');
            if (res && !res.headersSent) {
        ...
        conn.ev.on('creds.update', async () => {
            ...
            if (isNewSession) {
                huseLog(`🎉 NEW user ${sanitizedNumber} successfully registered!`, 'success');
            }
        });

        // Anti-delete
        conn.ev.on('messages.update', async (updates) => {
            await handleAntidelete(conn, updates, huseStore);
        });
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                await husemd(conn);
                huseLog(`Connected: ${sanitizedNumber}`, 'success');
            ...
            if (connection === 'close') {
                ...
                if (reason === DisconnectReason.loggedOut) huseLog(`Session logged out.`, 'error');
            }
        });
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        ...
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© HUSE-MD' } }
                    }},
                ...
                if (isCmd) {
                    ...
                    if (cmd) {
                        ...
                        } catch (e) { huseLog(`PLUGIN ERROR [${command}]: ${e.message}`, 'error'); }
                    }
                }
            ...
            } catch (e) { huseLog(`Message handler error: ${e.message}`, 'error'); }
        });

    } catch (err) {
        huseLog(`HUSE-MD Pair error: ${err.message}`, 'error');
...
router.get('/code', async (req, res) => { if (!req.query.number) return res.json({ error: 'Number required' }); await husePair(req.query.number, res); });
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'Huse-md is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 HUSE-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
...
async function autoReconnectFromMongoDB() {
    try {
        huseLog('Attempting auto-reconnect from MongoDB...', 'info');
        const numbers = await getAllNumbersFromMongoDB();
        if (!numbers.length) { huseLog('No numbers in MongoDB', 'info'); return; }
        for (const number of numbers) {
        ...
        huseLog('Auto-reconnect completed', 'success');
    } catch (e) { huseLog(`autoReconnectFromMongoDB error: ${e.message}`, 'error'); }
}
...
process.on('uncaughtException', (err) => {
    huseLog(`Uncaught exception: ${err.message}`, 'error');
});
